"""
Vision sanity check with three FIXED colored pillars (red, green, blue) placed
in front of the robot at reset. Unlike sim2sim_isaaclab_vision_carrot.py, the
pillars do NOT follow the robot — they are anchored to the world once and stay
put. Layout (from robot's POV, facing +x at spawn):
  - Red   : left   (+y)
  - Green : middle ( 0)
  - Blue  : right  (-y)

Usage:
  python scripts/sim2sim_isaaclab_vision_rgb_pillars.py \
...
parser = argparse.ArgumentParser(description="Run diffusion policy in Isaac Lab (G1) with three fixed RGB pillars in front")
...
parser.add_argument("--video_folder", type=str, default="videos/vision_rgb_pillars", help="Folder to save video")
...
# Pillar geometry / placement
parser.add_argument("--pillar_distance_m", type=float, default=1.5,
                    help="Distance from robot root (along its initial facing dir) to the pillar row.")
parser.add_argument("--pillar_spacing_m", type=float, default=0.6,
                    help="Lateral spacing between adjacent pillars.")
parser.add_argument("--pillar_radius_m", type=float, default=0.12,
                    help="Pillar radius in meters.")
parser.add_argument("--pillar_height_m", type=float, default=1.8,
                    help="Pillar height in meters. Tall enough to show up in head-mounted depth camera view.")
parser.add_argument("--pillar_z_center_m", type=float, default=0.9,
                    help="Pillar center height above ground.")
# Vision guidance (same as sim2sim_isaaclab_vision.py)
...
parser.add_argument("--vision_inpaint_image", type=str, default=None,
                    help="Path to reference RGB image. Inpaints the last horizon slot of the "
                         "vision stream with this embedding (v_t=0 every denoise step). "
                         "Propagates through the rolling schedule across act() calls. "
                         "Cheaper alternative to --vision_guidance_image; no scale tuning.")
parser.add_argument("--vision_inpaint_depth", type=str, default=None,
                    help="Optional reference depth (.npy, meters) paired with --vision_inpaint_image.")
...
# (name, color RGB, lateral offset sign). Lateral offset sign is multiplied by
# spacing. Robot's local left is +y when facing +x, so:
#   red   -> left   -> +1 * spacing
#   green -> middle ->  0
#   blue  -> right  -> -1 * spacing
PILLAR_SPECS = [
    ("pillar_red",   (0.9, 0.1, 0.1), 0.0),
    ("pillar_green", (0.1, 0.8, 0.1), +1.0),
    ("pillar_blue",  (0.1, 0.2, 0.9), -1.0),
]
...
def add_rgb_pillars_to_scene(env_cfg, radius_m: float, height_m: float):
    """Register three kinematic colored cylinders. Spawned far away; placed once after reset."""
    for name, color, _ in PILLAR_SPECS:
        setattr(env_cfg.scene, name, RigidObjectCfg(
    ...
    print(f"[SCENE] RGB pillar cfgs added: radius={radius_m:.3f} m, height={height_m:.3f} m", flush=True)


def place_rgb_pillars(env, distance_m: float, spacing_m: float, z_center_m: float, device: str, env_id: int = 0):
    """Anchor the three pillars in the world frame, computed from the robot's current root pose.
    Called once, right after reset — pillars stay fixed thereafter."""
    ...
    for name, _, lateral_sign in PILLAR_SPECS:
        if name not in scene.keys():
            print(f"[SCENE][WARN] '{name}' missing from scene, skipping.", flush=True)
            continue
        target_xy = root_pos_w[:2] + fwd * float(distance_m) + left * float(lateral_sign) * float(spacing_m)
        pillar = scene[name]
        state = pillar.data.root_state_w[env_id:env_id + 1].clone()
        ...
        state[0, 6] = 0.0
        state[0, 7:] = 0.0
        pillar.write_root_state_to_sim(state, env_ids=torch.tensor([env_id], device=device))
        print(f"[PILLAR] {name} placed at ({target_xy[0]:.3f}, {target_xy[1]:.3f}, {z_center_m:.3f})", flush=True)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg):
    ...
    add_rgb_pillars_to_scene(env_cfg, args_cli.pillar_radius_m, args_cli.pillar_height_m)
    ...
    # Detect whether the model was trained with depth (with_depth=true -> 2176-dim
    # vision stream of SigLIP RGB + DeFM depth) or RGB-only (with_depth=false -> 1152).
    model_vision_dim = int(getattr(policy.actor, "vision_dim", 0))
    use_depth = model_vision_dim == RGB_EMBED_DIM + DEPTH_EMBED_DIM
    if model_vision_dim == RGB_EMBED_DIM:
        print("[VISION] Model vision_dim=1152 — RGB-only (skipping DeFM).", flush=True)
    elif use_depth:
        print("[VISION] Model vision_dim=2176 — RGB + depth.", flush=True)
    else:
        print(f"[VISION][WARN] Unexpected model vision_dim={model_vision_dim}; "
              f"expected {RGB_EMBED_DIM} (RGB-only) or {RGB_EMBED_DIM + DEPTH_EMBED_DIM} (RGB+depth).",
              flush=True)

    print(f"[INFO] Loading vision encoders (SigLIP2{' + DeFM' if use_depth else ''})...", flush=True)
    siglip_model, siglip_processor, defm_model = load_vision_encoders(device, load_defm=use_depth)
    ...
    try:
        ...
        for name, _, _ in PILLAR_SPECS:
            if name not in scene_keys:
                print(f"[SCENE][ERROR] '{name}' is NOT in the scene — the cfg mutation was lost.", flush=True)
    except Exception as e:
        print(f"[SCENE] Could not list scene keys: {e}", flush=True)
    if isinstance(env.unwrapped, DirectMARLEnv):
        env = multi_agent_to_single_agent(env)
    if record_video:
        video_folder = os.path.abspath(os.path.expanduser(getattr(args_cli, "video_folder", "videos/vision_rgb_pillars")))
        video_length = getattr(args_cli, "video_length", 500)
        pillar_tag = f"pillars_d{args_cli.pillar_distance_m:.2f}m_s{args_cli.pillar_spacing_m:.2f}m"
        if args_cli.guidance_type:
            video_name = f"{model_name}_{pillar_tag}_fwd{args_cli.forward_speed}_lat{args_cli.lateral_speed}_spin{args_cli.spin_speed}_scale{args_cli.guidance_scale}"
        elif args_cli.vision_guidance_image is not None:
            video_name = f"{model_name}_{pillar_tag}_visionguidancescale{args_cli.vision_guidance_scale}"
        elif args_cli.vision_inpaint_image is not None:
            video_name = f"{model_name}_{pillar_tag}_visioninpainting"
        else:
            video_name = f"{model_name}_{pillar_tag}_noguidance"
    ...
    if args_cli.vision_guidance_image is not None:
        ...
        ref_rgb_emb = encode_rgb(ref_rgb, siglip_model, siglip_processor, device)

        depth_given = args_cli.vision_guidance_depth is not None
        if not use_depth:
            if depth_given:
                print("[VISION-GUIDANCE] Model is RGB-only; ignoring --vision_guidance_depth.", flush=True)
            ref_vec = ref_rgb_emb
        else:
            if depth_given:
                ref_depth = np.load(args_cli.vision_guidance_depth).astype(np.float32)
                ref_depth_emb = encode_depth(ref_depth, defm_model, device)
            else:
                ref_depth_emb = np.zeros(DEPTH_EMBED_DIM, dtype=np.float32)
            ref_vec = np.concatenate([ref_rgb_emb, ref_depth_emb])

        ref_raw = torch.from_numpy(ref_vec.astype(np.float32)).to(device).view(1, 1, -1)
        ref_normed = policy.actor.normalize_vision(ref_raw).squeeze(0).squeeze(0)

        # `rgb_only` slices the predicted vision to its RGB portion before comparing.
        # For an RGB-only model, the whole embedding IS RGB, so no slicing is needed.
        if not use_depth:
            rgb_only = False
        else:
            rgb_only = args_cli.vision_guidance_rgb_only or not depth_given
    ...
    vision_goal_embed = None
    if args_cli.vision_inpaint_image is not None:
        from PIL import Image
        print(f"[VISION-INPAINT] Encoding reference image: {args_cli.vision_inpaint_image}", flush=True)
        ip_rgb = np.array(Image.open(args_cli.vision_inpaint_image).convert("RGB"))
        ip_rgb_emb = encode_rgb(ip_rgb, siglip_model, siglip_processor, device)
        if not use_depth:
            ip_vec = ip_rgb_emb
        else:
            if args_cli.vision_inpaint_depth is not None:
                ip_depth = np.load(args_cli.vision_inpaint_depth).astype(np.float32)
                ip_depth_emb = encode_depth(ip_depth, defm_model, device)
            else:
                ip_depth_emb = np.zeros(DEPTH_EMBED_DIM, dtype=np.float32)
            ip_vec = np.concatenate([ip_rgb_emb, ip_depth_emb])
        ip_raw = torch.from_numpy(ip_vec.astype(np.float32)).to(device).view(1, 1, -1)
        vision_goal_embed = policy.actor.normalize_vision(ip_raw).squeeze(0).squeeze(0)
        print(f"[VISION-INPAINT] goal embed shape={tuple(vision_goal_embed.shape)}", flush=True)
    ...
    # Anchor the pillars in the world frame ONCE, based on where the robot spawned.
    place_rgb_pillars(
        env,
        args_cli.pillar_distance_m,
        args_cli.pillar_spacing_m,
        args_cli.pillar_z_center_m,
        device,
    )
    ...
    if debug_vision:
        debug_vision_dir = os.path.abspath(
            os.path.join(getattr(args_cli, "video_folder", "videos/vision_rgb_pillars"), "vision_debug")
        )
    ...
    while step_count < max_steps and simulation_app.is_running():
        ...
        # else:
        #     vision_embeds = rgb_emb.astype(np.float32)
        vision_embeds = vision_goal_embed.cpu().numpy().astype(np.float32)

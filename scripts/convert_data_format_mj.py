...
qpos = robot_data.qpos.copy()
qvel = robot_data.qvel.copy()

log["qpos"].append(qpos)
log["qvel"].append(qvel)

# Save only the hinge DOFs
log["joint_pos"].append(qpos[dof_qpos_start : dof_qpos_start + n_dof].copy())
log["joint_vel"].append(qvel[dof_qvel_start : dof_qvel_start + n_dof].copy())

...
def force_based_push(
    ...
    Bodies listed in force_push_no_push_on_contact are not pushed when they are in ground contact
    (current_air_time == 0).
    """
    ...
    # Zero out forces on grounded feet (hard mask via current_air_time)
    if command.force_push_no_contact_indices and contact_sensor_name in env.scene.sensors:
        contact_sensor = env.scene.sensors[contact_sensor_name]
        for fp_idx, body_id in zip(command.force_push_no_contact_indices, command.force_push_no_contact_body_ids):
            grounded = contact_sensor.data.current_air_time[:, body_id] == 0.0  # (num_envs,)
            forces[grounded, fp_idx] = 0.0

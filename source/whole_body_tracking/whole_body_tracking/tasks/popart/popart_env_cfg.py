...
import whole_body_tracking.tasks.popart.mdp as mdp
...
class ObservationsCfg:
    ...
    @configclass
    class CategoryCfg(ObsGroup):
        """PopArt-only group: per-env category index. Sits in obs/storage so the
        PopArt-aware ActorCritic can gather it; not routed to policy/critic torsos."""

        category_idx = ObsTerm(func=mdp.category_idx_obs, params={"command_name": "motion"})
    ...
    category: CategoryCfg = CategoryCfg()
...
@configclass
class PopartEnvCfg(ManagerBasedRLEnvCfg):

...
class ZarrMotionLoader:
    ...
    def __init__(self, zarr_path: str, body_indexes: Sequence[int], device: str = "cpu",
        ...
        # categorizer (used by MultiClipMotionCommandPopArt).
...
# ─── PopArt multi-clip command ───────────────────────────────────────────────
# Adds a per-env category index, derived from clip name via a named categorizer.
# For Stage A this is plumbing only — `category_idx` is just exposed as an
# observation. Stage B will consume it inside a custom ActorCritic / PPO.

from whole_body_tracking.tasks.popart.mdp.categorizers import (  # noqa: E402
    CATEGORIZERS,
...
class MultiClipMotionCommandPopArt(MultiClipMotionCommand):
    """Multi-clip motion command that also exposes a per-env category index
    (one int per env identifying the broad behavior class — walk / jog / ...).
    ...
    cfg: "MultiClipMotionCommandPopArtCfg"

    def __init__(self, cfg: "MultiClipMotionCommandPopArtCfg", env: "ManagerBasedRLEnv"):
        # If the user gave a `categories` list, derive include_motion_types
        # from it BEFORE the parent loads the zarr (so unmatched clips never
        # get loaded). Auto-derive num_categories from the list length too.
        # Categories list takes precedence over the legacy `categorizer` field.
        if cfg.categories is not None and len(cfg.categories) > 0:
        ...
        if unmatched and cfg.unmatched == "raise":
            ...
            raise ValueError(
                f"[PopArt] {len(unmatched)} clip(s) unmatched by categorizer "
        ...
        # Resolve human-readable names for downstream logging. Dynamic
        # `categories` takes priority; otherwise look up the named categorizer's
        # registered names (with fallback to cat_0, cat_1, ...).
...
@configclass
class MultiClipMotionCommandPopArtCfg(MultiClipMotionCommandCfg):
    """Configuration for the PopArt multi-clip motion command."""

    class_type: type = MultiClipMotionCommandPopArt
    ...
    """Legacy API: name of a function in `popart.mdp.categorizers.CATEGORIZERS`.

...
from whole_body_tracking.tasks.popart.mdp.commands import MotionCommand
...
# ── PopArt: per-env category index ────────────────────────────────────────────

def category_idx_obs(env: ManagerBasedEnv, command_name: str = "motion") -> torch.Tensor:
    """Per-env category index, shape (num_envs, 1) as float (storage requires float).

    Lives in its own observation group so the standard ActorCritic torsos
    don't see it. Read by the PopArt-aware ActorCritic (Stage B) by casting
    back to long.
    """

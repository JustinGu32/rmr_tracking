"""Named clip-name → category-index functions used by `MultiClipMotionCommandCategorized`.
...
For data-driven categories (Phase 5), use `make_latent_kmeans_categorizer`
to build one from a clusters JSON produced by
`scripts/cluster_motion_latents.py`.
"""
...
# ─── Phase 5: latent-kmeans categorizer ────────────────────────────────────

def make_latent_kmeans_categorizer(centroids_path: str) -> Callable[[str], int]:
    """Build a data-driven categorizer from the JSON produced by
    `scripts/cluster_motion_latents.py`.

    The JSON maps each clip name to a cluster id. Returns a function that
    looks up `clip_name → cluster_id`; raises ValueError on a name not in
    the JSON or with the -1 sentinel (clip was too short to encode).
    """
    with open(centroids_path, "r") as f:
        data = json.load(f)
    cluster_id_by_name: dict[str, int] = data["cluster_id_by_name"]
    k = int(data["k"])

    def _fn(clip_name: str) -> int:
        if clip_name not in cluster_id_by_name:
            raise ValueError(
                f"clip {clip_name!r} not in latent_kmeans centroids file "
                f"{centroids_path!r}; re-run dump_motion_latents.py on the "
                f"current zarr to refresh."
            )
        c = cluster_id_by_name[clip_name]
        if c < 0 or c >= k:
            raise ValueError(
                f"clip {clip_name!r} has cluster_id={c} (sentinel for too-short "
                f"clip or unassigned). Increase --per_clip_window_stride or "
                f"shorten --window during latent dump."
            )
        return int(c)

"""Execute the pinned trainer with only the continuity-runner import changed."""
...
CONTINUITY_IMPORT = (
    "from experiments.ppo_resume_continuity.runner import "
    "ResumeContinuityMotionOnPolicyRunner as OnPolicyRunner"
)
...
def patch_stable_trainer_source(source: str) -> str:
    ...
    return source.replace(ORIGINAL_IMPORT, CONTINUITY_IMPORT)
...
def main() -> None:
    ...
    if observed_sha != EXPECTED_STABLE_TRAINER_SHA256:
        raise RuntimeError(
            "stable trainer hash drift: "
            f"expected {EXPECTED_STABLE_TRAINER_SHA256}, observed {observed_sha}"
        )
    ...
    print(f"[RESUME-CONTINUITY-CODE] whole_body_tracking={observed_origin}", flush=True)
    globals_dict = {
        "__name__": "__main__",

...
def test_pinned_stable_trainer_gets_only_runner_import_replaced():
    source = STABLE_TRAINER.read_text(encoding="utf-8")
    assert (
        hashlib.sha256(STABLE_TRAINER.read_bytes()).hexdigest()
        == EXPECTED_STABLE_TRAINER_SHA256
    )
    patched = patch_stable_trainer_source(source)
    assert patched != source
    assert patched.count("FirstUpdateProbeMotionOnPolicyRunner as OnPolicyRunner") == 1
    assert "MotionOnPolicyRunner as OnPolicyRunner" not in patched.replace(
        "FirstUpdateProbeMotionOnPolicyRunner as OnPolicyRunner", ""
    )

"""Execute the pinned stable trainer with one in-memory runner import change."""
...
def patch_stable_trainer_source(source: str) -> str:
    """Replace exactly the runner-selection line and reject source drift."""
    count = source.count(ORIGINAL_IMPORT)
    if count != 1:
        raise RuntimeError(
            f"expected stable runner import exactly once, observed {count}"
        )
    return source.replace(ORIGINAL_IMPORT, CORRECTED_IMPORT)
...
def main() -> None:
    ...
    if observed_sha != EXPECTED_STABLE_TRAINER_SHA256:
        raise RuntimeError(
            f"stable trainer hash drift: expected {EXPECTED_STABLE_TRAINER_SHA256}, "
            f"observed {observed_sha}"
        )
    source = patch_stable_trainer_source(trainer_bytes.decode("utf-8"))
    scripts_dir = str(STABLE_TRAINER.parent)
